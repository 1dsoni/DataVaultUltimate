...
public class SelectImageHelper {
    ...
    private void beginCrop(final Uri source) {
        final Uri destination = Uri.fromFile(new File(context.getCacheDir(), "cropped"));
        //Crop.of(source, destination).asSquare().start(this);
       // Crop.of(source, destination).withAspect(1, 1).withMaxSize(200, 200).asSquare().start(context);
        //Crop.of(source, destination).withAspect(1,1).withMaxSize(1280, 768).withAspect(1280,768).start(context);
        final CharSequence[] option = {"Icon", "As Square","fit Landscape","fit Portrait", "Landscape", "Portrait"};
        checkPermission(context);
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle("Choose Crop Style");
        builder.setItems(option, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                if (option[which]=="As Square"){
                    cropSquare(source,destination);
                }
                else if (option[which]=="Icon"){
                    cropIcon(source,destination);
                }
                else if (option[which]=="Landscape"){
                    cropLandscape(source,destination);
                }
                else if (option[which]=="Portrait"){
                    cropPortrait(source,destination);
                }
                else if (option[which]=="fit Landscape"){
                    cropFitLandscape(source,destination);
                }
                else if (option[which]=="fit Portrait"){

                    cropFitPortrait(source,destination);
                }
            }
        });
    ...
    private void cropFitLandscape(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1, 1).withMaxSize(1024, 768).withAspect(hieght, width).start(context);
    }

    private void cropFitPortrait(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1, 1).withMaxSize(768, 1024).withAspect(width, hieght).start(context);
    }

    private void cropIcon(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1, 1).withMaxSize(200, 200).asSquare().start(context);
    }

    private void cropSquare(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1, 1).withMaxSize(768, 768).asSquare().start(context);
    }

    private void cropLandscape(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1,1).withMaxSize(1024, 768).withAspect(1980,1080).start(context);
    }

    private void cropPortrait(Uri source, Uri destination){
        checkPermission(context);
        Crop.of(source, destination).withAspect(1,1).withMaxSize(768, 1024).withAspect(1080,1920).start(context);
    }
